...
function weatherResponseCallback(callback, error, weatherResponse, condition, city, state, date, WU_API_KEY, debug) {
    ...
    if (error) {
        ...
        if (weatherResponse.status == 'successful') {
            ...
            if (!(condition == 'UV' && weatherResponse.forecast)){
            ...
            callback({tell: weatherReply}, city, locationUtil.getStateName(state));
            ...
            if(debug) console.log("Got " + weatherResponse.us_cities.length + " cities " + " named " + city + " in the US.");
            ...
            callback({ask: ["I found "+state_names.length+" states with a city named "+city+":",state_string,"Which state are you interestd in?"], options:state_names});
        } else {
